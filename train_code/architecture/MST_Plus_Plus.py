...
def ReLU_attention(Q, K, V):
    ...
    attention_scores = F.relu(Q) @ torch.diag(1. / torch.arange(1, Q.size(0) + 1).float())

    # Compute the weighted sum of values
    weighted_sum = attention_scores @ (F.relu(K).transpose(0, 1) @ V)

    # Normalize the result
    normalization_factor = attention_scores.sum(dim=1, keepdim=True)
...
class MS_MSA(nn.Module):
    ...
    def forward(self, x_in):
        ...
        #使用ReLU_attention替换原来的softmax
        ReLU_attention(q, k, v)
